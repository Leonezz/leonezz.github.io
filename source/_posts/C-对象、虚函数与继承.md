...
C++ 中的继承是不满足「零代价抽象」原则的, 具体地说，C++ 对象使用了额外的内存保存关于基类信息才实现了运行时多态. 但这种额外信息究竟以何种方式在对象的内存中报错对我来说仍是未知. 受到前人文章的启发, 本文借助 GDB 调试器一步步地观察记录 C++ 对象的内存布局, 力求理解实际 C++ 编译器 (gcc 9.4.0) 对 C++ 对象的内存布局以及派生类的实现.
...
运行时多态的实现得益于每个对象内存中的虚表: 调用虚函数时, 只需要查询虚表然后决定要调用哪个版本的实现就可以实现多态. 使用基类指针时, 随着基类指针所指向的对象的不同, 调用虚函数时所查询的虚表自然也不同.

按道理说, 虚表存在于对象的内存中, 那么只要及时更新对象, 使用基类的对象而非指针应该也可以是实现运行时多态. 即下面的代码:
